...
(async () => {
    ...
    while (!isPageLoaded) {
        try {
            let response = await page.goto('https://www.bestbuy.ca/en-ca/product/zotac-nvidia-geforce-rtx-3080-trinity-oc-10gb-gddr6x-video-card/15000077', { timeout: 30000 });
            if (response.status() !== 404) {
                isPageLoaded = true;
            }
        } catch (err) {
            try {
                await page.goto('https://www.bestbuy.ca/en-ca/collection/rtx-30-series-graphic-cards/316108?path=category%253AComputers%2B%2526%2BTablets%253Bcategory%253APC%2BComponents%253Bcategory%253AGraphics%2BCards%253Bcustom0graphicscardmodel%253AGeForce%2BRTX%2B3080', { timeout: 30000 });
                await page.click('text=ZOTAC NVIDIA GeForce RTX 3080 Trinity OC 10GB GDDR6X Video Card', { timeout: 30000 });
                isPageLoaded = true;
            } catch (err) {
                console.error(err);
